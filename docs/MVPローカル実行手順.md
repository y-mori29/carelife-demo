...
**本番環境**: バックエンド・フロントエンド・LINE Bot は Cloud Run にデプロイ済みで、環境変数は GCP の「変数とシークレット」に登録されています。**動きの確認だけなら**、README の「動きの確認（フロントエンドのみ・本番 API 利用）」のとおり、フロントのみ起動して本番バックエンドを向ければよく、`.env` やバックエンドの起動は不要です。

**このドキュメント**: ローカルで **バックエンドも含めて**「録音 → Speech-to-Text → Gemini 要約 → フロントに表示 → LINE に送信」まで動かす場合の手順です。バックエンドの開発・デバッグや、GCP を使わないモック以外の動作確認をするときに参照してください。
